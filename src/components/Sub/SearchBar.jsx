    ...
    return (
        <div>
            ...
            <div className="flex items-center text-gray-600 focus-within:text-gray-400">
                <input
                    placeholder="Search movies & more"
                    className={`py-2 text-base text-black bg-white rounded-md pl-4 focus:outline-none focus:bg-white focus:text-gray-900 duration-300 transition-all ${searchState ? "w-64 visible" : "w-0 invisible"}`}
                    value={query}
                    onChange={changeHandler}
                    on
                    required
                ...
                <BiSearchAlt2 onClick={() => setSearchState(!searchState)} size={30} className={`p-1 focus:outline-none focus:shadow-outline cursor-pointer ${searchState ? "" : "hover:text-white text-slate-200"}`} />

            </div>

            <div className="absolute mt-2 flex flex-col h-[300px] overflow-auto overflow-x-hidden w-64 rounded-md z-50">
                {autocompleteData?.results?.length > 0 ?
                    ...
                    autocompleteData?.results.map((eachItem) => (
                        <Button media_type={eachItem.media_type} id={eachItem.id} >
                            <div className="flex gap-2 w-64 h-auto p-2 bg-[#26262c] odd:bg-[#17171b] hover:text-[#EAB308] cursor-pointer">
                                ...
                                <div className="flex flex-col justify-center items-start">
                                    <p className="text-white text-sm text-left">{eachItem.title || eachItem.name}</p>
